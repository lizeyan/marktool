...
void DataEdit::resizeEvent(QResizeEvent*)
{
  refreshScales();
}
...
void DataEdit::updateYticks()
{
  ...
  for (int i=0; i<10; ++i) {
    qreal val = i * (ds_->max_pv() - ds_->min_pv()) * yPercent_ * 0.1 +
                ds_->min_pv();
...
//                    yPercent_);
  ...
  if (!diff_mode_) {
    ...
    for (int i=0; i<3; ++i) {
      ...
      QRect textBox(legendBox.left() + 70, legendBox.top() + 3 + i * 20, 60, 20);
      painter.drawText(textBox, Qt::AlignVCenter, labels[i]);

      QPointF p1(legendBox.left() + 15, textBox.top() + textBox.height() * 0.5),
          p2(p1.x() + 40, p1.y());
    ...
    for (int i=0; i<4; ++i) {
      ...
      QRect textBox(legendBox.left() + 70, legendBox.top() + 3 + i * 20, 30, 20);
      painter.drawText(textBox, Qt::AlignVCenter, labels[i]);
